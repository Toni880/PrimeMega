...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            ...
            return
        await lel.edit(f"Prime Mega AI Actived by {message.from_user.mention()} for users in {message.chat.title}")
        ...
        if not Escobar:
            ...
            return
        await lel.edit(f"Prime Mega AI Deactivated by {message.from_user.mention()} for users in {message.chat.title}")
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        ...
        test = test.replace("Prime", "Aco")
        test = test.replace("My god is @Bukan_guudlooking", "I'm a Muslim")
        test = test.replace("21", "9")
        test = test.replace("I'm single.")
        test = test.replace("I'm single.")
        test = test.replace("@PrimeSupportGroup", "Prime Mega.ai")
        test = test.replace("I live in @PrimeSupportGroup.", "I live in San Francisco, California.")
        ...
        test = test.replace(
            "Hi, My name is Prime Nice to meet you.",
        ...
        response = response.replace("luna", "Prime")
        response = response.replace("I'm a Christian", "My god is @Bukan_guudlooking")
        response = response.replace("9", "21")
        response = response.replace("I'm married to my job.")
        response = response.replace("I'm single.")
        response = response.replace("Prime Mega.ai", "@PrimeSupportGroup")
        response = response.replace("I live in San Francisco, California.", "I live in @PrimeSupportGroup.")
        ...
        response = response.replace("Luna", "Prime")
        response = response.replace("luna", "Prime")
        response = response.replace("I'm a Muslim", "My owner is @Bukan_guudlooking")
        response = response.replace("9", "16")
        response = response.replace("I'm married to my job.", "I'm married with @Bukan_guudlooking")
        response = response.replace("I'm single.")
        response = response.replace("Prime Mega.ai", "@PrimeSupportGroup")
        response = response.replace("I live in San Francisco, California.", "I live in @PrimeSupportGroup")
        response = response.replace("I made myself", "I was Created by @Bukan_guudlooking")
        response = response.replace(
                "Hi, my friend! Do you want me to tell you a joke?",
                "Hello there I am Prime.. nice to meet you")
...
async def inuka(client, message):
    ...
    test = test.replace("My god is @Bukan_guudlooking", "I'm a Muslim")
    test = test.replace("21", "9")
    test = test.replace("@Bukan_guudlooking is my husband.", "I'm single.")
    test = test.replace("@PrimeSupportGroup", "Prime Mega.ai")
    test = test.replace("I live in @PrimeSupportGroup.", "I live in San Francisco, California.")
    test = test.replace("I was created by @Bukan_guudlooking", "I made myself")
    test = test.replace(
    ...
    response = response.replace("Luna", "Prime")
    response = response.replace("luna", "Prime")
    response = response.replace("I'm a Muslim", "My god is @Bukan_guudlooking")
    response = response.replace("9", "21")
    response = response.replace("I'm married to my job.", "I'm married with @Bukan_guudlooking")
    response = response.replace("I'm single.", "My husband is @Bukan_guudlooking")
    response = response.replace("Prime Mega.ai", "@PrimeSupportGroup")
    response = response.replace("I live in San Francisco, California.", "I live in @PrimeSupportGroup")
    response = response.replace("I made myself", "I was Created by @Bukan_guudlooking")
    response = response.replace(
...
@pbot.on_message(filters.regex("Prime|PRIME|robot|Tonic|prime") & ~filters.bot & ~filters.via_bot  & ~filters.forwarded & ~filters.reply & ~filters.channel & ~filters.edited)
async def inuka(client, message):
    msg = message.text
    if msg.startswith("/") or msg.startswith("@"):
        message.continue_propagation()
    ...
    test = test.replace("Prime", "Aco")
    test = test.replace("Prime", "Aco")
    test = test.replace("My god is @Bukan_guudlooking", "I'm a Christian")
    test = test.replace("21", "9") 
    test = test.replace("@Bukan_guudlooking is my husband.", "I'm single.")
    test = test.replace("@PrimeSupportGroup", "Prime Mega.ai")
    test = test.replace("I live in @PrimeSupportGroup.", "I live in San Francisco, California.")
...
__help__ = """
❂ Prime Mega AI is the only ai system which can detect & reply upto 200 language's
